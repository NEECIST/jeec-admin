      ...
      <form class="col s12" @submit="updateEvent">
        ...
        <div class="row">
          <div class="input-field col s12">
            <input class="col s5 validate" id="name" name="name" type="text" required placeholder="Name"
              v-model="responsedata.event.name" maxlength="100">
          </div>
          ...
          <div class="row">
            <div class="input-field col s2">
              <p>Starting Date</p>
              <date-pick v-model="responsedata.event.start_date" :format="format" :parseDate="parseDate"
                :formatDate="formatDate" :inputAttributes="{ size: 32 }" required></date-pick>
            </div>


            <div class="input-field col s2">
              <p>Closing Date</p>
              <date-pick v-model="responsedata.event.end_date" :format="format" :parseDate="parseDate"
                :formatDate="formatDate" :inputAttributes="{ size: 32 }" required></date-pick>
            </div>
...
-->
        ...
        <div class="row">
          <div class="input-field col s5">
            <input class="col s12 validate" id="location" name="location" type="text" placeholder="Location"
              v-model="responsedata.event.location" maxlength="100">
          </div>

          <div class="input-field col s3">
            <input id="email" name="email" type="email" class="validate" placeholder="Contact email"
              v-model="responsedata.event.email" maxlength="100">
        ...
        <div class="row">
          <div class="input-field col s2">
            ...
            <input id="facebook_link" name="facebook_link" type="url" class="validate"
              v-model="responsedata.event.facebook_link" maxlength="100">
          </div>

          <div class="input-field col s2">
            <i class="material-icons left">event</i>Facebook event
            <input id="facebook_event_link" name="facebook_event_link" type="url" class="validate"
              v-model="responsedata.event.facebook_event_link" maxlength="100">
          </div>

          <div class="input-field col s2">
            <i class="material-icons left">movie</i>Youtube
            <input id="youtube_link" name="youtube_link" type="url" class="validate"
              v-model="responsedata.event.youtube_link" maxlength="100">
          </div>

          <div class="input-field col s2">
            <i class="material-icons left">camera</i>Instagram
            <input id="instagram_link" name="instagram_link" type="url" class="validate"
              v-model="responsedata.event.instagram_link" maxlength="100">
          </div>
        </div>
        ...
        <div class="row">
          <div class="input-field col s2">
            ...
            <date-pick v-model="responsedata.event.cvs_submission_start" :format="format" :parseDate="parseDate"
              :formatDate="formatDate" :inputAttributes="{ size: 32 }" required></date-pick>
          </div>

          <div class="input-field col s2">
            <p>CV's Submission End date</p>
            <date-pick v-model="responsedata.event.cvs_submission_end" :format="format" :parseDate="parseDate"
              :formatDate="formatDate" :inputAttributes="{ size: 32 }" required></date-pick>
          ...
          <div class="input-field col s2">
            ...
            <date-pick v-model="responsedata.event.cvs_access_start" :format="format" :parseDate="parseDate"
              :formatDate="formatDate" :inputAttributes="{ size: 32 }" required></date-pick>
          ...
          <div class="input-field col s2">
            ...
            <date-pick v-model="responsedata.event.cvs_access_end" :format="format" :parseDate="parseDate"
              :formatDate="formatDate" :inputAttributes="{ size: 32 }" required></date-pick>
        ...
        <div class="row">
          <div class="input-field col s2">
            ...
            <date-pick v-model="responsedata.event.end_game_day" :format="format" :parseDate="parseDate"
              :formatDate="formatDate" :inputAttributes="{ size: 32 }" required></date-pick>
        ...
        <div style="margin-left: -78vw;">
          ...
          <p>
            <label style="margin-right: 20px;">
              <input class="with-gap" type="radio" value="True" required
                v-model="responsedata.event.show_registrations" />
              <span>Yes</span>
            </label>
            <label>
              <input class="with-gap" type="radio" value="False" required
                v-model="responsedata.event.show_registrations" />
              <span>No</span>
            </label>
          </p>
        ...
        <button @click="updateEvent()" class="waves-effect blue lighten-2 btn add-btn left"><i
            class="material-icons left">save</i>Save Event</button>

        <button @click="deleteEvent()" class="waves-effect red lighten-2 btn add-btn left"><i
            class="material-icons left">delete</i>Delete Event</button>
      </form>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    await axios.post(process.env.VUE_APP_JEEC_BRAIN_URL + '/event/info', { external_id: this.$route.params.event_external_id }, {
      auth: {
  ...
  methods: {
    ...
    updateEvent() {
      ...
      axios.post(process.env.VUE_APP_JEEC_BRAIN_URL + '/update-event-vue', new_event, {
        auth: {
          username: process.env.VUE_APP_JEEC_WEBSITE_USERNAME,
          password: process.env.VUE_APP_JEEC_WEBSITE_KEY
        }
    ...
    deleteEvent() {
      ...
      axios.post(process.env.VUE_APP_JEEC_BRAIN_URL + '/delete-event-vue', new_event, {
        auth: {
          username: process.env.VUE_APP_JEEC_WEBSITE_USERNAME,
          password: process.env.VUE_APP_JEEC_WEBSITE_KEY
        }
